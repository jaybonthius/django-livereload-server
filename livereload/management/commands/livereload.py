...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--force",
            dest="ignore-patterns",
            action="store_true",
            help="Prevent watching matching directories",
        )
        parser.add_argument(
            "-w",
            "--wait",
            ...
            type=float,
            default=0,
            help="Delay before reloading page in seconds",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["ignore-template-dirs"] is not True:
            ...
            logger.info(f"TEMPLATE_DIRS: {getattr(settings, 'TEMPLATE_DIRS', [])}")
            for template in getattr(settings, "TEMPLATES", []):
                if "DIRS" in template:
                    watch_dirs.extend(template["DIRS"])
                    logger.info(f"TEMPLATES DIRS: {template['DIRS']}")
            logger.info(
                f"app_configs: {[os.path.join(app_config.path, 'templates')for app_config in app_configs]}"
            )
            watch_dirs.extend(
                [
                    os.path.join(app_config.path, "templates")
        ...
        if options["ignore-static-dirs"] is not True:
            ...
            watch_dirs.extend(
                [os.path.join(app_config.path, "static") for app_config in app_configs]
            )
        ...
        for dir in filter(None, watch_dirs):
            server.watch(
                str(dir), delay=options["delay"]
            )  # Watcher can only handle strings, not Path objects
